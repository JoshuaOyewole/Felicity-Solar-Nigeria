...
function AddProduct() {
    ...
    const onSubmit: SubmitHandler<FormSchema> = (data) => {
        if (!imageFiles[0] || !imageFiles[1] || !imageFiles[2]) {
            toast.error('Please select at least 3 product images.');
            return;
        }
        ...
        formData.append('total_stock', '1000');
        formData.append('key_features', keyFeatures);
        formData.append('discount_rate', data.discount);
        if (data.videoLink) formData.append('video_link', data.videoLink);
        formData.append('image_1', imageFiles[0]);
        formData.append('image_2', imageFiles[1]);
        formData.append('image_3', imageFiles[2]);
